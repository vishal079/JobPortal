...
namespace JobPortal.Areas.Employer.Controllers
{
	...
	/* This is method to Download Resume*/
        ...
        [Authorize]
        public ActionResult SendMailtoAll(string[] pCandidateId, string mailtoMarked, string deleteMarked, string messageAll)
        {
            HttpCookie cookie = HttpContext.Request.Cookies.Get(FormsAuthentication.FormsCookieName);
            if (cookie != null)
            {
                ...
                if (deleteMarked == "Delete Marked")
                {
                   ...
                   _employerRepository.RemoveMarkCandidate(ids,_employerVm.ObjJob);
                    ViewAll(_employerVm.ObjJob.Id);
                    return View("ViewAll");
                }
